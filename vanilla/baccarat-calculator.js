class BaccaratCalculator extends HTMLElement {
    ...
    setupEventListeners() {
        ...
        this._keydownHandler = (e) => {
            ...
            const postModal = this.shadowRoot.getElementById('post-modal-overlay');
            if (postModal && !postModal.classList.contains('hidden')) return;

            if (e.key === '+' || e.key === '=') {
                this.handleUnitChange(1);
            } else if (e.key === '-' || e.key === '_') {
                this.handleUnitChange(-1);
            } else if (e.key === '0') {
                this.handleUndoUnitChange();
            }
        };
        ...
        if (this.sessions.length === 0) {
            ...
            Object.entries(groupedSessions).forEach(([date, dateSessions]) => {
                ...
                dateSessions.forEach(session => {
                    ...
                    const modelText = session.model || 'MetaState';
